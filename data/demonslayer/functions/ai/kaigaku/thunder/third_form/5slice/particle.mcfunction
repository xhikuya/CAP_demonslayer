...
tag @s add immune
execute unless entity @s[predicate=demonslayer:demon] positioned ^ ^ ^2 positioned ~.7 ~.7 ~.7 as @e[tag=!immune,dx=-1.4,dz=-1.4,dy=-1.4,predicate=!core:dmg,nbt=!{HurtTime:10s},predicate=demonslayer:target_slayer] run scoreboard players operation @s dmg = #ds:temp var1
execute if entity @s[predicate=demonslayer:demon] positioned ^ ^ ^2 positioned ~.7 ~.7 ~.7 as @e[tag=!immune,dx=-1.4,dz=-1.4,dy=-1.4,predicate=!core:dmg,nbt=!{HurtTime:10s},predicate=demonslayer:target_demon] run scoreboard players operation @s dmg = #ds:temp var1
