...
execute if entity @s[predicate=demonslayer:demon,nbt={SelectedItem:{tag:{Cap:{Kaigaku:1b}}}}] positioned ^ ^ ^1.4 positioned ~.7 ~.7 ~.7 as @e[tag=!immune,type=!#core:non_living,dx=-1.4,dz=-1.4,dy=-1.4,predicate=core:dmg] run effect give @s wither 5 5 true
execute positioned ^ ^ ^2 positioned ~1 ~1 ~1 as @e[tag=!immune,type=!#core:non_living,dx=-2,dz=-2,dy=-2,predicate=core:dmg] at @s facing entity @e[tag=ds_this,limit=1,sort=nearest] feet run function demonslayer:breathing/thunder/kaigaku/fifth_form/knockback
