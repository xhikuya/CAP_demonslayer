execute as @s at @s unless entity @s[x_rotation=-90..-10] rotated ~ 0 if block ^ ^.3 ^.2 #movable if block ^ ^.3 ^.4 #movable if block ^ ^.3 ^.6 #movable if block ^ ^.3 ^.8 #movable anchored eyes if block ^ ^ ^.2 #movable if block ^ ^ ^.4 #movable if block ^ ^ ^.6 #movable if block ^ ^ ^.8 #movable anchored feet run tp @s ^.01 ^ ^0.7
