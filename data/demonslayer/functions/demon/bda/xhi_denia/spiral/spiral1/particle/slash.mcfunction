...
particle sweep_attack ~ ~ ~ 0.1 0.1 0.1 0 1 normal
tag @s add proj
tag @e remove immune
execute as @e if score @s uuid = @e[tag=proj,limit=1] projowner run tag @s add immune
execute positioned ~1.2 ~3 ~1.2 run scoreboard players operation @e[nbt={HurtTime:0s},dx=-2.4,dz=-2.4,dy=-6,tag=!immune] dmg = @s var2
tag @e remove immune
tag @s add proj
