...
#particle sweep_attack ~ ~ ~ 0 0 0 0 1 normal

tag @s add proj
tag @e remove immune
execute as @e if score @s uuid = @e[tag=proj,limit=1] projowner run tag @s add immune
execute positioned ~.5 ~1 ~.5 run scoreboard players operation @e[nbt={HurtTime:0s},dx=-1,dz=-1,dy=-2,tag=!immune] dmg = @s var2
tag @e remove immune
tag @s add proj
