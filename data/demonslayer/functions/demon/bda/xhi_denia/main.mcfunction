...
execute if predicate core:click unless score @s[scores={ds_bdamove=1}] stamina matches 40.. run tellraw @s "Too Tired"
execute if predicate core:click if score @s[scores={stamina=40..,ds_cooldown=0,ds_formtime=0}] ds_bdamove matches 1 rotated ~ 0 run function demonslayer:demon/bda/xhi_denia/spiral/init
